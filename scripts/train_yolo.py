...
def main():
    ...
    if args.data_yaml is None:
        print(f"\n📊 Preparing YOLO dataset with symlinks...")
        ...
        if data_yaml_path.exists() and (output_dir / 'images' / 'train').exists():
            print(f"   ✅ Using existing prepared dataset: {output_dir}")
        else:
            ...
            print(f"   ✅ Dataset prepared in {stats['time_seconds']:.1f}s")
    ...
    print(f"\n💡 Speed tips applied:")
    print(f"   • Using symlinks (no data duplication)")
    print(f"   • Image size: {args.imgsz} (smaller = faster)")
    print(f"   • Batch size: {args.batch_size}")
    print(f"   • Frozen backbone (less parameters)")
    if args.cache:
        print(f"   • Image caching enabled (faster epoch 2+)")
    if args.amp:
        print(f"   • Mixed precision training (faster on GPU)")
