...
- `bbox_utils.py` - Bounding box utilities (crop, format conversions, padding)
...
- `pinhole.py` - Pinhole camera model for 3D translation computation from bbox and depth
- `prepare_yolo_symlinks.py` - YOLO dataset preparation via symlinks (or copy)
- `training.py` - Centralized training loops for pose models
- `transforms.py` - Image transformations, quaternions, augmentation
- `validation.py` - Validation pipeline and result analysis functions
- `visualization.py` - Visualization utilities for training, metrics, samples

## 3. Main Components

### `bbox_utils.py`
- **Purpose**: Handles common bounding box operations: centered crop, padding, format conversions (e.g., YOLO).
- **Main Functions**:
	- `crop_and_pad(img, bbox, output_size, margin)` - Square crop centered on bbox with optional padding.
	- `convert_bbox_to_yolo_format(bbox, img_width, img_height)` - Convert bbox to normalized YOLO format.
	- `crop_bbox_optimized(img, bbox_xyxy, margin, output_size)` - Optimized crop from bbox in xyxy format.
...
### `pinhole.py`
- **Purpose**: Implements the pinhole camera model to calculate 3D translation from bbox, depth map, and camera intrinsics.
- **Main Functions**:
...
### `transforms.py`
- **Purpose**: Image and pose transformations, conversions between rotation representations, augmentation.
- **Main Functions**:
	- `rotation_matrix_to_quaternion`, `quaternion_to_rotation_matrix_batch` - Conversions between rotation and quaternion.
	- `crop_image_from_bbox` - Crop images from bbox.
	- `get_pose_transforms` - Compose transformations for training/val.
